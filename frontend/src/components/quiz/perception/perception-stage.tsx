...
const descriptors = [
  { key: "p_1", label: "Professional" },
  { key: "p_2", label: "Approachable" },
  { key: "p_3", label: "Confident" },
  { key: "p_4", label: "Commanding" },
  { key: "p_5", label: "Effortless" },
  { key: "p_6", label: "Refined" },
  { key: "p_7", label: "Elite" },
  { key: "p_8", label: "Grounded" },
  { key: "p_9", label: "Dependable" },
  { key: "p_10", label: "Mature" },
  { key: "p_11", label: "Youthful" },
  { key: "p_12", label: "Romantic" },
  { key: "p_13", label: "Mysterious" },
  { key: "p_14", label: "Bold" },
  { key: "p_15", label: "Expressive" },
  { key: "p_16", label: "Subtle" },
  { key: "p_17", label: "Fresh" },
  { key: "p_18", label: "Light" },
  { key: "p_19", label: "Energizing" },
  { key: "p_20", label: "Rich" },
  { key: "p_21", label: "Attractive" },
  { key: "p_22", label: "Warm" },
];
...
export default function PerceptionStage() {
  ...
  const [answers, setAnswers] = useState<PerceptionState>(initialState);

  const handleToggle = (key: keyof PerceptionState) => {
    setAnswers((prev) => ({
      ...prev,
      [key]: !prev[key],
    }));
  ...
  const handleSubmit = async () => {
    if (selectedCount < 5) {
      setValidationError("Please select at least 5 descriptors.");
      return;
    }
    ...
    try {
      ...
      const response = await submitPerceptionStage(payload);
      console.log("PERCEPTION STAGE SUBMITTED", response);
      router.push("/quiz/personality");
    } catch (error) {
      console.log("FAILED TO SUBMIT PERCEPTION", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const fetchPerceptionStage = async () => {
      try {
        ...
        const response = await getPerceptionStage(sessionId);
        console.log("PERCEPTION RESTORE", response);
        ...
        console.log("NO EXISTING PERCEPTION DATA", error);
      }
    };
      ...
      <div className="mt-8 flex flex-wrap gap-2.5 flex-1 content-start">
        {descriptors.map((item) => (
          <button
            key={item.key}
            type="button"
            onClick={() => handleToggle(item.key as keyof PerceptionState)}
