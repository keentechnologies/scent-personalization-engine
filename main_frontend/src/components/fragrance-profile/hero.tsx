...
export function FragranceProfileHero() {
  return (
        ...
        <div className="grid grid-cols-1 items-center gap-12 md:grid-cols-[1.05fr_0.95fr] md:gap-14">
          <motion.div variants={containerVariants} initial="hidden" animate="visible">
            <motion.div variants={itemVariants}>
              <Eyebrow icon="✦" text="Your Profile" />
            </motion.div>

            <motion.h1 variants={itemVariants} className="mt-5">
              Your Fragrance Is More Than A Bottle.
            </motion.h1>

            <motion.div variants={itemVariants} className="mt-6">
              <p className="text-[clamp(17px,4vw,20px)] leading-relaxed text-text-muted max-w-[600px]">
                Behind every Crafted Sprays fragrance is a detailed fragrance profile designed to uncover what truly suits you.
              </p>
            </motion.div>

            <motion.div variants={itemVariants} className="mt-8 flex flex-wrap gap-3">
              <Button variant="primary" comingSoon>Start My Discovery →</Button>
              <Button variant="secondary" href="#sample">View Sample Profile</Button>
            </motion.div>
          </motion.div>
